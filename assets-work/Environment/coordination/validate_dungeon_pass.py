"""Validate the integrated Dungeon scope and exact texture resolution.
...
MODEL_PREFIX = DATA_PREFIX + 'Object2/'
...
def check_textures(converter):
    ...
    for model in sorted(folder.glob('*.bmd')):
        raw_info = subprocess.check_output([str(converter), 'info', str(model)])
        info = raw_info.decode('ascii', errors='backslashreplace')
