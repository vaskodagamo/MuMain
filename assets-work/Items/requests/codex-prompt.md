...
When "What to change" names a reference mesh (a .glb under ../item-sources/), build from it instead of modelling from scratch: import it into Blender, reduce it to the request's triangle limit (Decimate, then clean up the topology), bake its colour and detail onto one diffuse texture, and fit it to the original model's origin, grip, size and orientation. The mesh stays where it is: never copy it (or a .gltf export of it) into the repository; source.blend and exports/ hold only the reduced model (the validator refuses any delivery file over 8 MB).

Two checks before you deliver, because the offline previews hide both:
