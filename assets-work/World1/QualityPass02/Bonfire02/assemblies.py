...
from review import configure_shell
...
def group(label,records,scale_reference=False):
    ...
    for stage in ('current','candidate'):
        ...
        set_camera(bounds);set_lighting();configure_shell();bpy.context.scene.cycles.samples=16
...
if __name__=='__main__':
    group('small-tilted',[placement('Bonfire01',0)],True)
    group('medium-scale',[placement('Bonfire01',1)],True)
    group('adjacent-tilted-pair',[placement('Bonfire01',3),placement('Bonfire01',4)])
