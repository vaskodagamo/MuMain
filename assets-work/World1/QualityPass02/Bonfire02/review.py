...
def configure_shell(wood_only=False):
    for material in bpy.data.materials:
        if material.get('mu_texture') != 'fire_02.jpg':continue
        nodes,links=material.node_tree.nodes,material.node_tree.links
        texture=next(n for n in nodes if n.type=='TEX_IMAGE')
        output=next(n for n in nodes if n.type=='OUTPUT_MATERIAL')
        transparent=nodes.new('ShaderNodeBsdfTransparent')
        if wood_only:
            links.new(transparent.outputs[0],output.inputs['Surface']);continue
        emission=nodes.new('ShaderNodeEmission');emission.inputs['Strength'].default_value=.7
        add=nodes.new('ShaderNodeAddShader')
        links.new(texture.outputs['Color'],emission.inputs['Color'])
        links.new(transparent.outputs[0],add.inputs[0]);links.new(emission.outputs[0],add.inputs[1])
...
def review_asset(name):
    ...
    for stage,path in stages.items():
        open_scene(path,bounds);configure_shell(True)
        render(folder/'review'/(stage+'-wood.png'))
        bpy.context.scene.render.resolution_percentage=25
        render(folder/'review'/(stage+'-wood-small.png'))
    open_scene(folder/'validation/reimported.blend',bounds);configure_shell(True)
