"""Official Blender/BMD interchange and strict four-root broadleaf contract audits."""
...
def validate(name,shared,raw):
    ...
    assert 'meshes: 1' in info and 'texture=tree_09.tga' in info
    ...
    assert all(not m['vertex_to_normal_node_mismatch_corner_counts'] for m in binding['mesh_bindings']),binding
    (report/'raw-bindings.json').write_text(json.dumps(binding,indent=2))
    texture=folder/'exports/tree_09.OZT'
    assert texture.read_bytes()==(REPOSITORY/'src/bin/Data/Object1/tree_09.OZT').read_bytes()
    ...
    (report/'texture-check.txt').write_text(result.stdout)
    (report/'summary.json').write_text(json.dumps(dict(status='PASS',bounds_before=before,bounds_after=after,
        skeleton_actions='EQUIVALENT',mesh_order=['tree_09.tga'],client_verified=False),indent=2))
    import_model(folder,'exports')


def main():
    if sys.argv[1]=='prepare':
        for name in NAMES:import_model(ROOT/name,'baseline')
        return
    ...
    helper('final_corner_audit',ROOT/'audit_exports.py')
