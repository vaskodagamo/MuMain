...
def build(name):
    ...
    retained = max(min((v.co - w.co).length for w in obj.data.vertices) for v in source.data.vertices)
    assert retained < .0001, retained
    report=dict(original_control_vertices=len(source.data.vertices), max_control_vertex_distance=retained,
        control_contract='Every original edge control, including roots, tips and ground contacts retained', triangles=len(obj.data.polygons),bounds_before=before,bounds_after=after,
