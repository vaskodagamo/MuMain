...
NAMES=os.environ.get('FOUNTAIN_NAMES','Waterspout01').split(',')
...
def append(record,anchor,stage):
    ...
    path=ROOT/name/('baseline/source.blend' if stage=='baseline' else 'validation/reimported.blend') if name=='Waterspout01' else ROOT/'context'/name/'source.blend'
...
def group(label,data):
    ...
    for stage in ('baseline','candidate'):
        ...
        render(ROOT/'review'/f'{label}-{stage}-small.png')
    ...
    (ROOT/'review'/f'{label}-contacts.json').write_text(json.dumps(dict(kind='OFFLINE ACTUAL PLACEMENTS, NOT CLIENT',**data,bounds=bounds,omitted='Terrain/collision/baked lighting/unselected unrelated objects'),indent=2)+'\n')
