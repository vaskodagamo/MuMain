...
def decode(path):
 raw=Path(path).read_bytes();src=c.create_string_buffer(raw);dst=c.create_string_buffer(128*128*3);handle=lib.tjInitDecompress();assert handle
 try:assert lib.tjDecompress2(handle,src,len(raw),dst,128,0,128,0,2048)==0
 finally:lib.tjDestroy(handle)
 return np.frombuffer(dst.raw,dtype=np.uint8).reshape(128,128,3).astype(int)
r=Path(sys.argv[1]);old=decode(r/'original.jpg');new=decode(r/'patched.jpg');diff=np.abs(old-new).max(axis=2)
mask=np.asarray(Image.open(Path(__file__).resolve().parent/'artwork/input-references/protected-basin-plus2px.png'))>0
selected=np.frombuffer((r/'allowed-blocks.bin').read_bytes(),dtype=np.uint8).reshape(16,16).astype(bool);editable=np.repeat(np.repeat(selected,8,axis=0),8,axis=1)
assert diff[mask].max()==0 and diff[~editable].max()==0
report=dict(status='PASS',decoder='libturbojpeg tjDecompress2 TJPF_RGB TJFLAG_FASTDCT(2048), matching GlobalBitmap.cpp',protected_filter_margin_max_rgb_error=int(diff[mask].max()),all_unselected_blocks_max_rgb_error=int(diff[~editable].max()),no_mipmaps='GlobalBitmap.cpp144 texInfo.num_levels=1; no resampling for128power-of-two',sampler='LoadData.h defaultGL_NEAREST, GlobalBitmap.cpp211–214 min/mag filter andnearest mip mode;2pxguard also covers fallbackbilinear')
