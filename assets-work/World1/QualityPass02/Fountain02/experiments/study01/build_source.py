...
def sculpt(point,wing=False):
 ...
 if wing:
  ...
  leading=43-23*min(1,across/80)+65*max(0,(across-110)/(span-110))
  ...
  curve=6*math.exp(-((across-49)/22)**2)*(1-web)*smoothstep(t/.13)
  p.z+=min(curve,max(0,311.204803-p.z)*.6)
  p.y-=3*math.exp(-((across-49)/22)**2)*(1-web)*smoothstep(t/.13)
