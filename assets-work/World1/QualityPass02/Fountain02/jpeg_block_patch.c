/* Isolated lossless coefficient patcher. No engine/tool changes. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <jpeglib.h>
...
int main(int argc,char **argv){
 CHECK(argc==5,"usage: block-patch original.jpg candidate.jpg 16x16-mask.bin output.jpg");
 FILE *a=fopen(argv[1],"rb"),*b=fopen(argv[2],"rb"),*mask=fopen(argv[3],"rb");CHECK(a&&b&&mask,"input open failed");
 unsigned char allowed[256];CHECK(fread(allowed,1,256,mask)==256&&fgetc(mask)==EOF,"mask must be256bytes");fclose(mask);
 ...
 CHECK(original.image_width==128&&original.image_height==128&&candidate.image_width==128&&candidate.image_height==128,"requires128square");
 ...
 for(int c=0;c<3;c++){
  ...
  CHECK(x->width_in_blocks==16&&x->height_in_blocks==16&&y->width_in_blocks==16&&y->height_in_blocks==16,"block dimensions differ");
  ...
  for(int row=0;row<16;row++){
   ...
   for(int col=0;col<16;col++)if(allowed[row*16+col]){memcpy(dest[0][col],src[0][col],sizeof(JBLOCK));copied++;}
  }
 }
