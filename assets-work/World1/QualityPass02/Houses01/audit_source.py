...
def protected_contract(folder,actual):
    baseline=smd_triangles(folder/'baseline/smd'/f'{folder.name}.smd')
    remaining=list(actual)
    controlled=[(m,r) for m,r in baseline if m=='light_02.jpg' or (folder.name=='House04' and m!='tile_wood03.jpg')]
    for material,rows in controlled:match_triangle(material,rows,remaining)
    required={}
    edges={}
    for material,rows in baseline:
        for row in rows:
            if material!='tile_ston07.tga':required[(int(row[0]),tuple(row[1:4]))]=True
        if material=='tile_ston07.tga':
            for a,b in zip(rows,rows[1:]+rows[:1]):
                key=tuple(sorted((tuple(a[1:4]),tuple(b[1:4]))))
                edges.setdefault(key,[]).append((a,b))
    for records in edges.values():
        if len(records)!=1:continue
        for row in records[0]:required[(int(row[0]),tuple(row[1:4]))]=True
    vertices=root_points(actual)
    maximum=0
    for bone,position in required:
        delta=min(max(abs(a-b) for a,b in zip(position,q)) for q in vertices[bone])
        maximum=max(maximum,delta)
        assert delta<POSITION_TOLERANCE,(folder.name,bone,'protected original anchor',position,delta)
    return dict(protected_original_points=len(required),maximum_position_delta=maximum,controlled_light_triangles=len(controlled),awning_boundary_edges=sum(len(r)==1 for r in edges.values()),policy='Every original non-awning vertex and every awning perimeter endpoint retained; exact controlled light triangles preserved')
