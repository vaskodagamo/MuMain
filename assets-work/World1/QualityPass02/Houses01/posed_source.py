...
from raw_motion import motion,world
...
def match(texture,rows,actual):
    ...
    for index,(material,vertices) in enumerate(actual):
        ...
        for shift in range(3):
            ...
            if max(abs(x-y) for a,b in zip(rows,ordered) for x,y in zip(a[4:6],b[7:9]))>.00001:continue
            error=max(abs(x-y) for a,b in zip(rows,ordered) for x,y in zip(a[1:4],b[1:4]))
            if error<.002:candidates.append((error,index,shift))
    assert candidates
...
def audit():
    folder=ROOT/'House04';actual=actual_triangles(folder/'validation/new/House04.smd')
    ...
    assert not actual
    old_meta,old,_=motion(folder/'baseline/House04.bmd');new_meta,new,_=motion(folder/'exports/House04.bmd')
    ...
    for frame in range(40):
        ...
        assert maximum<.002,(frame,maximum)
        frames.append(dict(frame=frame,maximum_authored_corner_distance=maximum))
    (folder/'validation/posed-authored-match.json').write_text(json.dumps(dict(status='PASS',method='Every authored source triangle corner mapped one-to-one to final SMD corner, transformed through original intended raw baseline action and actual final raw BMD action at all40 keys',corners=len(pairs),frames=frames,maximum=max(f['maximum_authored_corner_distance'] for f in frames),tolerance=.002,normal_proof='raw-normal-bindings.json evaluates actual raw normal ownership/direction through every hierarchical frame'),indent=2)+'\n')
    print('Every authored source corner all40 poses PASS')
