...
def audit():
    folder=ROOT/'House04';before_path=folder/'baseline/House04.bmd';after_path=folder/'exports/House04.bmd'
    ...
    maximum=max(differences);assert maximum<.000001,maximum
    ...
    report=dict(status='PASS',raw_tail_byte_identical=old_tail==new_tail,baseline_raw_motion_sha256=hashlib.sha256(old_tail).hexdigest(),final_raw_motion_sha256=hashlib.sha256(new_tail).hexdigest(),maximum_raw_float_delta=maximum,raw_float_components=len(differences),action_metadata=old_meta,frames=frames,note='Exact baseline SMD motion inputs; raw byte identity is measured separately. Any sub-microfloat delta is converter decimal serialization, not tolerated Blender Euler drift.')
