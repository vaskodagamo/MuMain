"""Official Blender/BMD interchange and strict carved masonry contract audits."""
...
NAMES=tuple(sys.argv[2:]) if len(sys.argv)>2 else ('HouseEtc01',)
...
def main():
    if sys.argv[1]=='prepare':
        for name in ('HouseEtc01',):import_model(ROOT/name,'baseline')
        return
    ...
    helper('oriented_surface',ROOT/'audit_orientation.py').check()
    helper('protected_border',ROOT/'audit_border.py').check()
    helper('band_coverage',ROOT/'audit_band_coverage.py').check()
