...
def audit(name):
    ...
    report=dict(status='PASS',triangles=len(actual),one_to_one_triangle_match=True,area_metrics=actual_metrics,baseline_area_metrics=baseline_metrics,every_authored_triangle_matches_material_bone_position_uv=True,maximum_position_delta=maximum,position_tolerance=POSITION_TOLERANCE,uv_tolerance=UV_TOLERANCE,uv_layers=list(obj.data.uv_layers.keys()),packed_images={i.name:list(i.size) for i in images},per_root_contract=root_contract(folder,actual))
