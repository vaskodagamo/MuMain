...
import os
NAMES=tuple(os.environ.get('STATUE_NAMES','StoneStatue03').split(','))
...
def protected_contract(folder,actual):
    ...
    protected=[(m,r) for m,r in baseline if m=='tombstone_big.jpg'] if folder.name=='StoneStatue03' else []
    remaining=list(actual)
    for material,rows in protected:match_triangle(material,rows,remaining)
    return dict(exact_plinth_triangles=len(protected),policy='Angel plinth triangle/UV/bone identity; per-root extrema and all ground-contact vertices retained')
