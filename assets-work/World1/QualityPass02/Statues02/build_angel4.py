...
def feather_wings(output,obj):
 ...
 for side,ids in ((1,range(132,143)),(-1,range(231,242))):
  ...
  shapes=[([(10,12,204),(24,23,243),(45,28,254),(endpoint,24.2218,264.471985+offset)],9.5,3.2),
          ([(12,12,199),(27,25,224),(42,30,238),(60,28,243+offset)],9,3.6),
          ([(14,12,194),(26,26,207),(34,29,220),(47,29,224+offset)],8.5,3.8)]
  for layer,(controls,width,thickness) in enumerate(shapes):
   ...
   for t in (0,.2,.4,.6,.8,1):
    ...
    for j in range(6):
     ...
     ring.append(p)
     travel=t*(1-.22*layer)
     uv_ring.append(Vector((.843+.087*travel+.032*fullness*math.cos(angle),.1364+.4387*travel+.014*fullness*math.cos(angle)+.004*fullness*math.sin(angle))))
    rings.append(ring);uv_rings.append(uv_ring)
   for ring_index,(low,high) in enumerate(zip(rings,rings[1:])):
    for j in range(6):
     k=(j+1)%6;pts=[low[j],low[k],high[k],high[j]]
     pts.reverse()
     tex=[uv_rings[ring_index][j],uv_rings[ring_index][k],uv_rings[ring_index+1][k],uv_rings[ring_index+1][j]]
     tex.reverse();h.quad(output,pts,tex,0,1)
   pts=list(rings[0])
   cap_uv=uv_rings[0]
