...
def niche(output):
 # A carved arch surrounds the existing relief without moving or masking the painted figure.
 def path(width,base,spring,rise):
  return [Vector((-width,0,base)),Vector((-width,0,spring))]+[Vector((-width*math.cos(t*math.pi/8),0,spring+rise*math.sin(t*math.pi/8))) for t in range(1,8)]+[Vector((width,0,spring)),Vector((width,0,base))]
 outer=path(39,151,333,38);inner=path(32.7,157,333,30)
 rings=[]
 for points,y in ((outer,-6.2),(outer,-16),(inner,-18.5),(inner,-6.2)):
  rings.append([Vector((p.x,y,p.z)) for p in points])
 for first,second in zip(rings,rings[1:]):
  for i in range(len(first)-1):
   points=[first[i],first[i+1],second[i+1],second[i]]
   uv=[Vector((.11+p.x/350,.30+(p.z-150)/500)) for p in points]
   # Distinct UV area on narrow depth bands, using adjacent neutral painted stone.
   if abs(uv[0].x-uv[3].x)+abs(uv[0].y-uv[3].y)<1e-6:
    uv[2].x+=.015;uv[3].x+=.015
   h.quad(output,list(reversed(points)),list(reversed(uv)),0,0)
 for i in (0,len(outer)-1):
  points=[ring[i] for ring in rings]
  h.quad(output,points,[Vector((.1,.4)),Vector((.12,.4)),Vector((.12,.42)),Vector((.1,.42))],0,0)
...
def build(name):
 ...
 if name=='StoneStatue01':
  start=len(output);niche(output);normals.extend([Vector() for _ in range(3*(len(output)-start))])
...
if __name__=='__main__':
 for name in os.environ.get('STATUE_NAMES','StoneStatue01').split(','):build(name)
