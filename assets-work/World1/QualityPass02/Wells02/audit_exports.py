...
def same(a,b):
    ...
    return any(max(abs(x[k]-b[1][(i+shift)%3][k]) for i,x in enumerate(a[1]) for k in (0,1,2,3,7,8))<.0003 for shift in range(3))
def inspect(name):
...
if __name__=='__main__':main(('Well03','Well04','Well01'))
