...
from well_geometry import basin,roof
NAMES=('Well03','Well04','Well02','Well01')

def remodel(source,profiles,target,rig):
    ...
    for group in groups:
        ...
        if bone in profiles and len(group)==76:bodies.append((bone,group))
    ...
    removed=set().union(*(group for bone,group in bodies)) if bodies else set()
    ...
    records=[]
    for bone,group in bodies:
        world=[source.matrix_world@source.data.vertices[i].co for i in group]
        vs,fs,coords,record=vessel(profiles[bone],world)
...
def build(name):
    ...
    for source in reference.objects:
        obj,parts=remodel(source,profiles,target,rig);objects.append(obj);records.extend(parts)
