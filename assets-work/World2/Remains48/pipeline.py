"""Reproduce official imports, authored bone-remains study, exports and strict audits."""
...
def prepare():
    for name in ('Object48',):
        ...
        blender(folder / 'validation/roundtrip-export.txt', folder / 'baseline/source.blend',
                '--python', ROOT / 'official_io.py', '--', 'export', name, 'roundtrip')
        converter('bmd2smd', folder / 'roundtrip' / (name + '.bmd'), folder / 'validation/roundtrip')
        for stage in ('baseline', 'roundtrip'):
            for path in (folder / 'validation' / stage).glob('*.smd'):
                arguments = ['validate', path]
                if '_a' in path.stem:
                    arguments.append('--animation')
                (folder / 'validation' / (stage + '-' + path.stem + '-validate.txt')).write_text(converter(*arguments))
        from raw_audit import check
        assert check(folder, 'baseline')['status'] == 'PASS'
...
def validate(name):
    ...
    assert [line.split('texture=')[1] for line in info.splitlines() if 'texture=' in line] == ['bons.jpg']
    ...
    inspect(name)
    from audit_topology import main as check_topology
    check_topology()
