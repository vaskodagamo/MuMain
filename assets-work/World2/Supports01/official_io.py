...
def main():
    ...
    def run_converter(executable, *arguments):
        ...
        if values[0] == 'smd2bmd':
            ...
            # SourceTools represents the static root's +pi as -pi. Preserve the
            # original one-key track through the converter's supported manifest.
            while '--anim' in values:
                index = values.index('--anim')
                del values[index:index+2]
            values += ['--manifest', str(folder/'validation/baseline'/(name+'.actions.txt'))]
