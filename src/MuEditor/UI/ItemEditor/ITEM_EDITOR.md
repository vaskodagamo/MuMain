...
**Drawn** (how the game draws it, from `assets-work/Items/render-facts.json`: which meshes are blended
additively, alpha-blended or cut out, and the kinds of effects the engine adds, e.g.
`blended meshes 0 / effects: no level glow, animation` for the Wings of Elf; hover it for the notes of
items checked by hand), then every model file with triangles, meshes and textures and the original
commit.
...
6. **Scope** lists the files Codex may replace and the textures it must leave alone because other items
   ...
   a must-keep line that tells Codex how to paint it (for an additive mesh: paint on black, no opaque
   background); `brief.md` has it under **How the game draws this item**.
...
(an uncommitted change, or the catalog is out of date), the item already has an open request, or
`render-facts.json` is missing or has no entry for a target (rebuild it with
`python3 tools/item_editor/render_facts.py`; the validator rejects the request without it).
...
| Render facts (Drawn, Ask Codex) | `assets-work/Items/render-facts.json` in the checkout | How the game draws every mesh and what it adds; generated by `tools/item_editor/render_facts.py`. |
