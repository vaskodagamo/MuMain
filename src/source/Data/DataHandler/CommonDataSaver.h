...
public:
    ...
    template <typename TRuntime, typename TFile> struct SaveConfig
    {
        // File I/O
    ...
    // Generic save method
    template <typename TRuntime, typename TFile> static bool SaveData(const SaveConfig<TRuntime, TFile>& config);
...
private:
    ...
    static bool BuildBackupName(const wchar_t* fileName, int backupNumber, wchar_t* outBackupName,
